"""Seasonal Decoder — 중주파 성분을 direct 예측 후 cubic 보간."""
...
class SeasonalDecoder(nn.Module):
    """Flatten → MLP → n_coeffs 출력 → cubic spline 보간 → wave_s."""
    ...
    def forward(self, embeddings: torch.Tensor) -> torch.Tensor:
        ...
        if self.n_coeffs == self.horizon:
            return coeffs
        wave_s = _cubic_interpolate(coeffs, self.horizon)
        return wave_s


def make_seasonal_target(residual: torch.Tensor, pool_kernel: int) -> torch.Tensor:
    """학습 타겟 생성: AvgPool1d로 스무딩 후 cubic 보간으로 원래 길이 복원."""
    if pool_kernel <= 1:
        return residual
    H = residual.shape[1]
    pooled = F.avg_pool1d(residual.unsqueeze(1), kernel_size=pool_kernel)
