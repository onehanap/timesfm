"""April-Cov L1: 외인변수 + AvgPool 타겟 분해 + L1 loss."""
...
class ETTCovDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ctx_end = idx + self.context_len; fut_end = ctx_end + self.horizon
        return (torch.tensor(self.target[idx:ctx_end]),
                ...
                torch.tensor(self.covariates[idx:ctx_end]),
                torch.tensor(self.covariates[ctx_end:fut_end]))
    ...
    while step < max_steps:
        for context, masks, future, cov_ctx, cov_fut in train_loader:
            ...
            if stage == 1:
                ...
                r2_n = future_n - wave_t_n - wave_s_n
...
def evaluate(model, loader, device):
    ...
    for context, masks, future, cov_ctx, cov_fut in loader:
        context = context.to(device); masks = masks.to(device); future = future.to(device)
        cov_ctx = cov_ctx.to(device); cov_fut = cov_fut.to(device)
...
def evaluate_baseline(bl, test_ds, horizon, context_len):
    ...
    for i in range(len(test_ds)):
        c, _, f, _, _ = test_ds[i]; ctxs.append(c.numpy()); futs.append(f.numpy())
    preds = []
...
def run_single_horizon(cfg, df, device, baseline_model):
    ...
    if os.path.exists(nc_path):
        ...
        nl = DataLoader(_T(train_ds), batch_size=bs, shuffle=True, num_workers=2)
        train_nhits(nm, nl, cfg, device)
        tl = DataLoader(_T(test_ds), batch_size=bs, shuffle=False, num_workers=2)
        nhits_mse, nhits_mae, nhits_preds, _, nhits_decomps = evaluate_nhits(nm, tl, device)
