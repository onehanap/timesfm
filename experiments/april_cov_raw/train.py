"""April-Cov Raw: 외인변수 + Raw 타겟(보간 해상도 분해) + L1 loss."""
...
from april_cov.model.decomp_timesfm import DecompTimesFM
from april_cov.config import DEFAULT_CONFIG
...
LOSS_FN = F.l1_loss
LOSS_NAME = "Cov_Raw"
COV_COLS = ["HUFL", "HULL", "MUFL", "MULL", "LUFL", "LULL"]
...
class ETTCovDataset(Dataset):
    def __init__(self, df, context_len, horizon, target_col="OT", cov_cols=COV_COLS):
        self.context_len = context_len
        self.horizon = horizon
        self.target = df[target_col].values.astype(np.float32)
        self.covariates = df[cov_cols].values.astype(np.float32)
    ...
    def __getitem__(self, idx):
        ...
        return (
            torch.tensor(self.target[idx:ctx_end]),            # context [C]
            torch.zeros(self.context_len, dtype=torch.bool),   # masks [C]
            torch.tensor(self.target[ctx_end:fut_end]),         # future [H]
            torch.tensor(self.covariates[idx:ctx_end]),         # cov_context [C, 6]
            torch.tensor(self.covariates[ctx_end:fut_end]),     # cov_future [H, 6]
        )
...
def train_stage(model, stage, train_loader, cfg, device):
    ...
    for p in model.cov_encoder.parameters(): p.requires_grad = True

    losses = []
    step = 0
    while step < max_steps:
        for context, masks, future, cov_ctx, cov_fut in train_loader:
            if step >= max_steps: break
            context = context.to(device)
            masks = masks.to(device)
            future = future.to(device)
            cov_ctx = cov_ctx.to(device)
            cov_fut = cov_fut.to(device)

            fused, cov_fut_enc, mu, sigma = model._encode_and_fuse(
                context, masks, cov_ctx, cov_fut,
            )
            future_n = model._normalize_future(future, mu, sigma)

            if stage == 1:
                wave_t_n = model.decoder_t(fused, cov_future=cov_fut_enc)
                loss = LOSS_FN(wave_t_n, future_n)

            elif stage == 2:
                with torch.no_grad():
                    wave_t_n = model.decoder_t(fused, cov_future=cov_fut_enc)
                wave_s_n = model.decoder_s(fused, cov_future=cov_fut_enc)
                loss = LOSS_FN(wave_s_n, (future_n - wave_t_n).detach())

            elif stage == 3:
                with torch.no_grad():
                    wave_t_n = model.decoder_t(fused, cov_future=cov_fut_enc)
                    wave_s_n = model.decoder_s(fused, cov_future=cov_fut_enc)
                residual_n = model.decoder_r(fused, cov_future=cov_fut_enc)
                loss = LOSS_FN(residual_n, (future_n - wave_t_n - wave_s_n).detach())
...
def evaluate(model, loader, device):
    ...
    all_p, all_f, all_d = [], [], []
    for context, masks, future, cov_ctx, cov_fut in loader:
        context = context.to(device); masks = masks.to(device)
        future = future.to(device)
        cov_ctx = cov_ctx.to(device); cov_fut = cov_fut.to(device)
        pred, decomp = model(context, masks, cov_ctx, cov_fut)
        total_mse += ((pred - future)**2).sum().item()
        total_mae += (pred - future).abs().sum().item()
        n += future.numel()
        all_p.append(pred.cpu()); all_f.append(future.cpu())
...
def evaluate_baseline(bl, test_ds, horizon, context_len):
    ...
    for i in range(len(test_ds)):
        c, _, f, _, _ = test_ds[i]
        ctxs.append(c.numpy()); futs.append(f.numpy())
    preds = []
    ...
    return float(np.mean((preds-futs)**2)), float(np.mean(np.abs(preds-futs))), \
...
def train_nhits(nhits_model, train_loader, cfg, device):
    ...
    while step < total_steps:
        for batch in train_loader:
            ...
            context, masks, future = batch[0].to(device), batch[1].to(device), batch[2].to(device)
            loss = LOSS_FN(nhits_model(context, masks), future)
...
@torch.no_grad()
def evaluate_nhits(model, loader, device):
    model.eval()
    total_mse, total_mae, n = 0., 0., 0
    all_p, all_f, all_d = [], [], []
    for batch in loader:
        ctx, msk, fut = batch[0].to(device), batch[1].to(device), batch[2].to(device)
        pred, decomp = model(ctx, msk, return_decomposition=True)
...
def plot_decomposition(futures, horizon, ap, ad, np_, nd, n_samples=4):
    ...
    for row, idx in enumerate(ids):
        ...
        for col, (preds, decomps, title) in enumerate([
            ...
            for k, c in zip(decomps.keys(), ["#e74c3c","#3498db","#2ecc71"]):
...
def plot_comparison(futures, bl_p, ap, np_, horizon):
    ...
    for ax, idx in zip(axes.flat, ids):
        ...
        ax.set_title(f"Sample {idx} (Cov={ma:.2f}, NHiTS={mn:.2f})")
...
def run_single_horizon(cfg, df, device, baseline_model):
    ...
    train_ds = ETTCovDataset(df.iloc[:TRAIN_BORDER], context_len, horizon)
    test_ds = ETTCovDataset(df.iloc[VAL_BORDER:], context_len, horizon)
    ...
    nc_path = os.path.join(cache_dir, f"nhits_cache_h{horizon}.pt")
    if os.path.exists(nc_path):
        print(f"\n[N-HiTS] 캐시 로드")
        nc = torch.load(nc_path, map_location="cpu", weights_only=False)
        nhits_mse, nhits_mae, nhits_preds, nhits_decomps = nc["mse"], nc["mae"], nc["preds"], nc["decomps"]
    else:
        print("\n[N-HiTS] 학습 중...")
        nm = TimesFMWithNHiTSDecoder(horizon=horizon, context_len=context_len,
            n_blocks_per_stack=2, hidden_dim=512, dropout=0.0, unfreeze_last_n=0).to(device)
        # N-HiTS는 외인변수 없이 target만으로 학습
        from torch.utils.data import Dataset as _DS
        class _TargetOnly(_DS):
            def __init__(self, ds): self.ds = ds
            def __len__(self): return len(self.ds)
            def __getitem__(self, i):
                c, m, f, _, _ = self.ds[i]; return c, m, f
        nhits_loader = DataLoader(_TargetOnly(train_ds), batch_size=bs, shuffle=True, num_workers=2)
        train_nhits(nm, nhits_loader, cfg, device)
        nhits_test_loader = DataLoader(_TargetOnly(test_ds), batch_size=bs, shuffle=False, num_workers=2)
        nhits_mse, nhits_mae, nhits_preds, _, nhits_decomps = evaluate_nhits(nm, nhits_test_loader, device)
        torch.save({"mse":nhits_mse,"mae":nhits_mae,"preds":nhits_preds,"decomps":nhits_decomps}, nc_path)
    ...
    if os.path.exists(bl_path):
        ...
        baseline_model.compile(ForecastConfig(max_context=context_len, max_horizon=max_h,
...
def main():
    ...
    update_benchmark(f"April {LOSS_NAME}", results, steps_per_stage=cfg["max_steps_per_stage"])
